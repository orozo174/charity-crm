...
import { CATEGORIES, REGIONS, PRIORITY_CONFIG } from "../utils/constants";
...
export default function AddPerson({ setPage, onAdded }) {
  ...
  const validateStep = () => {
    ...
    if (step === 2) return form.region && form.address.trim();
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    // Медианы жаңы адамга жүктөө (эгер тандалса)
        ...
        <Card className="p-5 sm:p-6">
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <div>
                <label className={labelCls}>Облусу/шаары *</label>
              ...
              <div>
                <label className={labelCls}>Толук дареги *</label>
                <textarea className={`${inputCls} resize-none h-20`} placeholder="Дареги: көчөсү, үй номери..." value={form.address} onChange={(e) => update("address", e.target.value)} />
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              <p className="text-xs text-slate-400">
                Жооптуу волонтер катары сиз — <strong>{user?.full_name}</strong> — белгиленесиз.
              </p>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea className={`${inputCls} resize-none h-28`} placeholder="Бул адамдын кырдаалы жана эмне жардам керек экени жөнүндө жаз..." value={form.description} onChange={(e) => update("description", e.target.value)} />
              </div>
